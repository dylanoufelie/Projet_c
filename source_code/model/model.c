        ...
        if (sscanf(buffer, "%s %d %d", chain, &x, &y) == 3) {
            ...
            if (strcmp(chain, "NB_SOLAR_SYSTEM") == 0) {
                ...
                if (!(system->solarsystem[i].planet)) {
                    ...
                    system->solarsystem[i].planet[count2].radius = x;
                }
...
void Planets_Update(System* system, Uint32 deltaTime, int time) {
    if (!system || !system->solarsystem) {
        exit;
    }
    bool sign = 1;
    int angular = 0;

    for (int i = 0; i < system->nbsolarsystem; i++) {
        sign = 1;
        angular = 0;

        for (int j = 0; j < system->solarsystem[i].nbPlanets; j++) {
            printf("before for system %d planete %d , X = %d, Y = %d\n",i,j, system->solarsystem[i].planet[j].pos.x, system->solarsystem[i].planet[j].pos.y);
            if (system->solarsystem[i].planet[j].orbit < 0) {
                sign = 0;
            }
            angular = 2 * M_PI / system->solarsystem[i].planet[j].radius;

            if (sign == 0) {
                angular = -angular;
            }

            system->solarsystem[i].planet[j].pos.x = system->solarsystem[i].star.pos.x + abs(system->solarsystem[i].planet[j].orbit) * cos(angular * time - M_PI / 2); 
            system->solarsystem[i].planet[j].pos.y = system->solarsystem[i].star.pos.y + abs(system->solarsystem[i].planet[j].orbit) * sin(angular * time - M_PI / 2);

            printf("after for system %d planete %d , X = %d, Y = %d\n", i, j, system->solarsystem[i].planet[j].pos.x, system->solarsystem[i].planet[j].pos.y);
        }
    }
